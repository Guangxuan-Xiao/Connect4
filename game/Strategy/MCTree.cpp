...
int MCTree::bestMove(int node) {
	...
	double log_N = log(nodes[node].cnt + 0.01);
	for (int i = 0; i < initPhase.N; ++i) {
		...
		double score =
			nodes[child].score() +
			UCB_C * sqrt(log_N / nodes[child].cnt);
		if (score > maxScore) {
			maxScore = score;
			ret = i;
		}
...
void MCTree::setPhase(const Phase& phase, int move) {
	initPhase = phase;
	curPhase = phase;
	if (lastMove == -1 || (NODEPOOL_SIZE - nodes.size) <= 1e6) {
		nodes.size = 0;
...
//		if (j < mid) weight = j / 2 + 1;
//		else weight = (moveNum - 1 - j) / 2 + 1;
//		for (int k = i; k < i + weight; ++k)
//			adjustedMove[k] = j;
//		i += weight;
//	}
