...
void *callGetPoint(void *p_param) {
    ...
    try {
        param->p =
            param->getPoint(param->M, param->N, param->top, param->board,
                            param->lastX, param->lastY, param->noX, param->noY);
    } catch (Exception::BaseException &err) {
        // rls@2020-03-19: Add this Exception to prevent interruption
        cout << "**CRITICAL** error occurs when " << param->player
             << " getPoint:" << err.what() << endl;
        param->bugOccurred = 1;
        return NULL;
    } catch (...) {
        // rls@2020-03-19: Frankly speaking, I doubt if it really works, but I'm
...
// returns : 0 - 平局结束 1 - A赢 2 - B赢 3 - A出错 4 - A给出非法落子 5 - B出错
// 6 - B给出非法落子 7 - A超时 8 - B超时 -1 - 游戏未结束
int AGo(GETPOINT getPoint, CLEARPOINT clearPoint, Data *data) {
    ...
    try {
        ...
        // rls@2020-03-19: Add this Exception to prevent interruption
            ...
            cout << "**CRITICAL** error occurs when A clearPoint:" << err.what()
                 << endl;
            pthread_detach(threadID);  //释放资源
            return 3;
        }
...
int BGo(GETPOINT getPoint, CLEARPOINT clearPoint, Data *data) {
    ...
    try {
        ...
        // rls@2020-03-19: Add this Exception to prevent interruption
            ...
            cout << "**CRITICAL** error occurs when B clearPoint:" << err.what()
                 << endl;
            pthread_detach(threadID);  //释放资源
            return 3;
        }
...
/*
 ...
 reutrns:
 0 - 平局结束 1 - A赢 2 - B赢 3 - A出错 4 - A给出非法落子 5 - B出错 6 -
 B给出非法落子 7 - A超时 8 - B超时 -1 - A文件无法载入 -2 - B文件无法载入 -3 -
 A文件中无法找到需要的接口函数 -4 - B文件中无法找到需要的接口函数
 */
int compete(char strategyA[], char strategyB[], bool Afirst, Data *data) {
    ...
    if (getPointA == NULL || clearPointA == NULL) {
        cout << "Can't find entrance of the wanted functions in the A so file"
             << endl;
        return -3;
    }
    if (getPointB == NULL || clearPointB == NULL) {
        cout << "Can't find entrance of the wanted functions in the B so file"
             << endl;
        return -4;
    }
